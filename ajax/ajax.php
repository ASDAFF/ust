...
switch ($_REQUEST['FormType'])
{
                ...
                if ($ID = $el->Add($arCallback))
                {
                    if (!empty($PROP["TIME"]))
                    {
                        $times = CIBlockPropertyEnum::GetList(
                                        array(), array("IBLOCK_ID" => CALLBACK, "PROPERTY_ID" => "TIME", "ID" => $PROP["TIME"])
                        );
    ...
    case 'FileSend':
        ...
        {
            //if($USER->IsAdmin()){
            if (!empty($_REQUEST["PROPS_FIO"]) && !empty($_REQUEST["PROPS_email"]))
            {
                $el = new CIBlockElement;
                foreach ($_REQUEST as $key => $arProperts)
                {
                ...
                if ($ID = $el->Add($arCallback))
                {
                    $arSend = array(
                        "FIO" => $PROP["FIO"],
                        "EMAIL" => $PROP["email"],
                        "PERSONAL_NUMBER" => $PROP["personal_number"]
                    );
                    require_once($_SERVER["DOCUMENT_ROOT"] . "/mail.php");
                    $fileLink = $_SERVER["DOCUMENT_ROOT"] . "/" . $PROP["FILE_LINK"];
                    $m = new Mail; // начинаем 
                    $m->From($PROP["email"]); // от кого отправляется почта 


                    $m->To($PROP["email"]);

                    $arFilename = explode("/", $PROP["FILE_LINK"]);
                    $fileName = $arFilename[count($arFilename) - 1];
                    //1cmaster@argosgrp.ru, vera@argosgrp.ru, syrex88@gmail.com, edo-reg@argosgrp.ru
                    $today = date("Ymd");
                    //$name_wss = $today.": файл ".$fileName;
                    $name_wss = "Скачивание файла «" . $fileName . "» с сайта " . $_SERVER["HTTP_HOST"];
                    $txt_fields = "";
                    $m->Subject($name_wss);
                    $m->Body($txt_fields);
                    ...
                    //$m->Attach($filezip); // прикрепленный файл 
                    $m->Attach($fileLink, "", "", "attachment"); // прикрепленный файл 
                    // s$m->Attach($rtf->rtffile); // прикрепленный файл
                    //
                        //  $m->smtp_on("smtp.asd.com", "login", "password"); // если указана эта команда, отправка пойдет через SMTP 
                    $m->Send();    // а теперь пошла отправка            
                    //отправка xml на почту
                    require_once($_SERVER["DOCUMENT_ROOT"] . "/xmlfiledown.php");
                    // file_put_contents($_SERVER["DOCUMENT_ROOT"]."/test11.txt", "1");
                    $today1 = gettimeofday();
                    $dirtoday = implode("", $today1);
                    $uploaddir = $_SERVER["DOCUMENT_ROOT"] . '/filedown/' . $dirtoday . "/";
                    $xml = new xmlFiledown($uploaddir, $PROP);
                    $xml->saveXml();
                    // file_put_contents($_SERVER["DOCUMENT_ROOT"]."/test11.txt", "2");
                    $f = new Mail; // начинаем 
                    $f->From($PROP["email"]); // от кого отправляется почта 


                    $f->To("Spare@ust-co.ru");
                    $f->Cc("syrex88@gmail.com");

                    //1cmaster@argosgrp.ru, vera@argosgrp.ru, syrex88@gmail.com, edo-reg@argosgrp.ru
                    $today = date("Ymd");
                    // $name_wss = str_replace("'", "", $PROP["fullName_org_info"]);
                    // $name_wss = str_replace("\"", "", $name_wss);
                    // $name_wss = $today . "-" . $PROP["inn_org_info"] . "-" . $name_wss;
                    $name_wss = "Скачивание файла «" . $fileName . "» с сайта " . $_SERVER["HTTP_HOST"];
                    $txt_fields = "";
                    $f->Subject($name_wss);
                    $f->Body($PROP["email"]);
                    ...
                    //$m->Priority(3);    // приоритет письма
                    //$m->Attach($filezip); // прикрепленный файл 
                    $f->Attach($xml->xmlfile, "", "", "attachment"); // прикрепленный файл 
                    // $m->Attach($rtf->rtffile); // прикрепленный файл
                    //
                        //  $m->smtp_on("smtp.asd.com", "login", "password"); // если указана эта команда, отправка пойдет через SMTP 
                    $f->Send();    // а теперь пошла отправка            
                    // CEvent::Send("WEB_FORM", "s1", $arSend, "Y", 69);
                    $arResult["STATUS"] = 1;
                }
                else
    ...
    case "checkCatalogCount":
        ...
        {
            if ($_REQUEST["View"] > 0)
                $section_id = $_REQUEST["View"];
            else
                $section_id = $_REQUEST["Type"];
