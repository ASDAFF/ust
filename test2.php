<?  require($_SERVER["DOCUMENT_ROOT"]."/bitrix/header.php");
	...
	if(empty($arData)) {$fileError = 1;
		...
		foreach($arData["XML"]["#"]["Модели"][0]["#"]["Модель"] as $arModel){
			foreach($arModel["#"] as $key=>$attr){
				...
				} else $arResult[$count][$key] = $attr[0]["#"];
			}
			array_push($arXML_IDs, $arModel["#"]["XML_ID"][0]["#"]);
			$count++;
		}
		if(empty($arResult)) $xmlError = 1;
		else {
				...
				foreach($arSection["#"]["Разделы"][0]["#"]["Раздел"] as $arSubSection){
					...
					$arSectList = $dbSectList->Fetch();
						...
						if(empty($arSectList)){
							...
							if(!in_array($CHILD_SECTION_ID, $arSectXML_IDs)) $arSectXML_IDs[$arSections[$countRoot]["CHILDREN"][$countChildren]["XML_ID"]] = $CHILD_SECTION_ID;
							$countChildren++;
						}	
				}
			}
			...
			while($arElem = $dbElemList->GetNext()){
				...
				// if(!in_array($arElem["XML_ID"], $arXML_IDs)) {
		...
		// echo "</pre>";
				...
				$dbElemList = CIBlockElement::GetList(array("SORT"=>"ASC"), array("IBLOCK_ID"=>"54", "XML_ID"=>$arItem["XML_ID"], "ACTIVE"=>"Y"));
				while($arElemList = $dbElemList->Fetch()){
					if(!empty($arElemList)) {
						$dublCnt = 0;
						$dbprops = CIBlockElement::GetProperty(54, $arElemList["ID"]);
				...
				$arLoadProductArray = Array(
				  ...
				  "NAME"           => $newName,
				  "CODE" 		   => CUtil::translit($arItem["Название"], "ru", array("replace_space"=>"_", "replace_other"=>"")),
				  "ACTIVE"         => "Y",
				  "PREVIEW_TEXT"   => "",
	...
	//пишем логи
	if($fileError) $fileErrorText = "Ошибка файла";
	if($xmlError) $xmlErrorText = "Ошибка XML";
	$logContent = date("H:i:s").": Товаров в файле: ".$count." Товаров добавлено: ".$okCount." Товаров деактивировано: ".$deactivCount." Ошибок добавления: ".$neokCount." Ошибки: ".$error." ".$fileErrorText." ".$xmlErrorText."\n";
	...
	// if($importFile){
	// 	if($fileError + $xmlError > 0) rename($dir.$importFile, $_SERVER["DOCUMENT_ROOT"]."/1c/in_bad/".$importFile);
	// 	else rename($dir.$importFile, $_SERVER["DOCUMENT_ROOT"]."/1c/in_history/".$importFile);
	// }
