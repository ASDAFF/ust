...
<div class="title"></div>
<form name="filesend" method="post" action="<?=$APPLICATION->GetCurPage()?>" class="ajax">
    <input type="hidden" name="FormType" value="FileSend" />
    ...
    <?foreach($arResult["PROPS"] as $arProps):?>
        <?switch($arProps["PROPERTY_TYPE"]){
            case 'S': 
                ...
                ?>
                <?if($arProps["CODE"] != "FILE_LINK" && $arProps["CODE"] != "personal_number"):?>
                    <div class="line <?if($arProps["CODE"] == "TOWN"):?>autocomplete small<?endif?> fileform">
                ...
                <?else:?>
                    <input type="hidden" name="PROPS_<?=$arProps["CODE"]?>" value="" />
                <?endif;?>
                <?   
                }
            break;
    ...
    <div class="line"><button type="submit" name="send">Скачать файл</button></div>
