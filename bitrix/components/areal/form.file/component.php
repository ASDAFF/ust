...
if(CModule::IncludeModule("iblock")) {
    ...
    $cache_time = 3600;
    $arResult = array();
    $cache_dir_id = 'fileformparams';
    $cache_dir_path = '/fileformparams/';
    if ($cache_time > 0 && $cache->InitCache($cache_time, $cache_dir_id, $cache_dir_path)) {
       $res = $cache->GetVars();
       if (is_array($res["fileformparams"]) && (count($res["fileformparams"]) > 0))
        ...
        if ($cache_time > 0) {
            ...
            $cache->EndDataCache(array("fileformparams" => $arResult));
        }
    }
