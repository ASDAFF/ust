...
if ($this->StartResultCache(false, ($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups())))
{
    ...
    {
       // pr($arParams["ELEMENT_CODE"]);
        $groups = CIBlockElement::GetList(array(), array("IBLOCK_ID" => GROUPING_ELEMENT, "ACTIVE" => "Y", "CODE" => $arParams["ELEMENT_CODE"]), false, false, array("IBLOCK_ID", "ID", "NAME", "CODE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE"));
        if ($group = $groups->GetNextElement())
        {
            ...
            if (!empty($arSeries))
            {
                $serieses = CIBlockElement::GetList(array(), array("IBLOCK_ID" => SERIES_ELEMENT, "ACTIVE" => "Y", "ID" => $arSeries), false, false, array("ID", "PROPERTY_PHOTO", "PROPERTY_VIDEO", "PROPERTY_SCHEMES"));
                while ($series = $serieses->GetNext())
                {
                    /* Фотографии */
                    if (!empty($series["PROPERTY_PHOTO_VALUE"]))
                        $arPhoto = $series["PROPERTY_PHOTO_VALUE"];
                    if (!empty($series["PROPERTY_SCHEMES_VALUE"]))
                        $arSchemes = $series["PROPERTY_SCHEMES_VALUE"];
                    /* Видео */
        ...
        {
            $serieses = CIBlockElement::GetList(array(), array("IBLOCK_ID" => SERIES_ELEMENT, "ACTIVE" => "Y", "CODE" => $arParams["ELEMENT_CODE"]), false, false, array("IBLOCK_ID", "ID", "NAME", "CODE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE"));
            if ($series = $serieses->GetNextElement())
            {
                ...
                if (!empty($arProps["SCHEMES"]["VALUE"]))
                    $arSchemes = $arProps["SCHEMES"]["VALUE"];
        ...
        if (!empty($arPhoto))
        {
            foreach ($arPhoto as $photo)
            {
                ...
                $photos["RESIZE"] = CFile::ResizeImageGet(
                                $photo, array("width" => 62, "height" => 62), BX_RESIZE_IMAGE_EXACT, true
                );
        ...
        if (!empty($arSchemes))
        {
            ...
            {
                unset($schemes);
                $schemes["NATURE"] = CFile::GetPath($scheme);
                $schemes["STANDART"] = CFile::ResizeImageGet(
                                $scheme, array("width" => 388, "height" => 278), BX_RESIZE_IMAGE_PROPORTIONAL, true
                );
                $arResult["SCHEMES"][] = $schemes;
            }
        }
