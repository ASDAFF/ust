					...
					<div class="element element_<?=$key?> <?=$class?>">
						<?foreach($Dealer as $arDealer):?>
							...
							<?if(!empty($arDealer["PHONE"])):?>
                                                                <p><span class="town-num"Тел: <br /><?=$arDealer["PHONE"]?></span></p>
							<?endif;?>
							...
							<?if(!empty($arDealer["WEB_SITE"])):?>
								<p><a href="http://<?=$arDealer["WEB_SITE"]?>" target="_blank" rel="nofollow" title="<?=$arDealer["NAME"]?>"><?=$arDealer["WEB_SITE"]?></a></p>
							<?endif;?>
