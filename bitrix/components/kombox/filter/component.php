...
$arParams["FILTER_NAME"] = $_SESSION["CATALOG_FILTER_NAME"];
$arParams["IBLOCK_TYPE"] = $_SESSION["CATALOG_IBLOCK_TYPE"];
$arParams["IBLOCK_ID"] = $_SESSION["CATALOG_IBLOCK_ID"];
...
$newpageURL = explode("/", $pageURL);
$pagenewURL = array_splice($newpageURL, -1);
$pageURL = implode("/", $newpageURL);
$pageURL .= "/";
if ($_REQUEST["THIS_SECTION"])
{
    $pageURL = $_REQUEST["THIS_SECTION"];
}
//if($USER->IsAdmin()){
...
{
    $arResult["ELEMENT_COUNT"] = $arResult["ELEMENTS_COUNT"];
}
if ($arResult['IS_SEF'])
{
...
if (isset($_REQUEST["ajax"]) && $_REQUEST["ajax"] === "y")
{
    ...
    //echo '<pre>'; print_r($sefURL); echo '</pre>';
    if ($arResult['IS_SEF'])
    {
...
/* Добавляем ссылки  на домены в разделы */
 

$domains =get_domains();
$section = CIBlockSection::GetList(array(), array("IBLOCK_ID" => CATALOG, "ID" => $arParams["SECTION_ID"], "ACTIVE" => "Y", "NAME" => $arItem["TEXT"]), false, array("UF_URL"));
$name_domain = "ust-co.ru";
if ($section_UF = $section->GetNext())
{
    //print "<pre UF_URL style='display:none'>";
    // print_r($section_UF);
    // print "</pre>";
    ...
    if ($section_UF["UF_URL"] != "" and isset($domains[$section_UF["UF_URL"]]))
    {
        ...
        /* print "<pre UF_URL style='display:none'>";
          //print_r($domains[$section_UF["UF_URL"]]);
          print_r($section_UF["UF_URL"]);
          print "</pre>"; */
    }
    else
    {
...
print "<input type='hidden' id='urldomain' value='$name_domain'/>";
//print "<pre  FORM_ACTION  style='display:none'>"; print_R($arParams["SECTION_ID"]); print "</pre>";
...
if (
        $arParams["XML_EXPORT"] === "Y" && $arResult["SECTION"] && ($arResult["SECTION"]["RIGHT_MARGIN"] - $arResult["SECTION"]["LEFT_MARGIN"]) === 1
)
{
