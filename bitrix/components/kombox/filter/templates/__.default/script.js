...
$(document).ready(function(){
	...
	$('.kombox-filter-property-name').on('click', function(){
		var $this = $(this);
		...
		if(ul.length){
			...
			if(li.hasClass('closed')){
				...
				$.cookie('kombox-filter-closed-' + li.data('id'), false, { path: '/' });
			}
			else
			{
				li.addClass('closed');
				$.cookie('kombox-filter-closed-' + li.data('id'), true, { path: '/' });
			}
		}
		return false;
	});
	
	$('.kombox-filter li.closed ul').slideToggle(0);
	...
	if(result.ITEMS)
	{
		for(var PID in result.ITEMS)
		{
			...
			if(arItem.PROPERTY_TYPE == 'N' || arItem.PRICE)
			{
				...
				var slider = control.parent().find('.kombox-range div');
