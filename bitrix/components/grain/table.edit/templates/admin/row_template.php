...
?>
<input type="text" name="<?=$name?>" value="<?=htmlspecialchars($value)?>" size="<?=$arColumn["SIZE"]?>" />
<?
...
?>
<textarea name="<?=$name?>" cols="<?=$arColumn["COLS"]?>" rows="<?=$arColumn["ROWS"]?>"><?=htmlspecialchars($value)?></textarea>
<?
...
<select<?if($arColumn["MULTIPLE"]=="Y"):?> name="<?=$name?>[]" multiple="multiple"<?else:?> name="<?=$name?>"<?endif?>>
	<?foreach($arColumn["VALUES"] as $option):?>
		...
		?>
		<option value="<?=htmlspecialchars($option["VALUE"])?>"<?if($sel):?> selected="selected"<?endif?>><?=$option["LANG"][LANGUAGE_ID]?></option>
	<?endforeach?>
</select>
<?
...
?>
<input type="text" size="16" id="grain_table_date_field_<?=$id?>" name="<?=$name?>" value="<?=htmlspecialchars($value)?>" size="<?=$arColumn["SIZE"]?>" />
<?
...
?>
<input type="text" name="<?=$name?>" id="grain_table_fp_field_<?=$id?>" value="<?=htmlspecialchars($value)?>" /><?if($arResult["USE_FILEMAN"] && $arColumn["ALLOW_FILEMAN"]=="Y"):?>&nbsp;<a href="#" onclick="grain_table_tmp_input_id='grain_table_fp_field_<?=$id?>'; _grain_Table_OpenFM(); return false;" title="<?=GetMessage("GRAIN_TABLES_TE_TEMPLATE_STRUCTURE")?>"><img class="grain-tables-table-edit-file-icon" src="/bitrix/images/fileman/medialib/tabs/server.gif" width="16" height="16" border="0" alt="<?=GetMessage("GRAIN_TABLES_TE_TEMPLATE_STRUCTURE")?>" /></a><?endif?><?if($arResult["USE_MEDIALIB"] && $arColumn["ALLOW_MEDIALIB"]=="Y"):?>&nbsp;<a href="#" onclick="grain_table_tmp_input_id='grain_table_fp_field_<?=$id?>'; _grain_Table_OpenML(); return false;" title="<?=GetMessage("GRAIN_TABLES_TE_TEMPLATE_MEDIALIB")?>"><img class="grain-tables-table-edit-file-icon" src="/bitrix/images/fileman/medialib/tabs/media.gif" width="16" height="16" border="0" alt="<?=GetMessage("GRAIN_TABLES_TE_TEMPLATE_MEDIALIB")?>" /></a><?endif?>
...
<span class="<?=$arColumn["TEMPLATE_IDENTIFIER"]?>-values<?if($arColumn["MULTIPLE"]=="Y"):?>-multiple<?endif?> <?=$arColumn["TEMPLATE_IDENTIFIER"]?>-values<?if($arColumn["MULTIPLE"]=="Y"):?>-multiple<?endif?>-admin" id="grain_table_link_field_<?=$id?>_values">
    <?$i=0;foreach($SELECTED as $val=>$arItem):?>
    	<div class="<?=$arColumn["TEMPLATE_IDENTIFIER"]?>-values<?if($arColumn["MULTIPLE"]=="Y"):?>-multiple<?endif?>-value" id="<?=$arColumn["INSTANCE_IDENTIFIER"]?>_grain_table_link_field_<?=$id?>_value_<?=$val?>">
    		...
    		<a class="<?=$arColumn["TEMPLATE_IDENTIFIER"]?>-values<?if($arColumn["MULTIPLE"]=="Y"):?>-multiple<?endif?>-value-delete" href="#" onclick="<?=$arColumn["TEMPLATE_IDENTIFIER"]?>.deleteitem('<?=$arColumn["INSTANCE_IDENTIFIER"]?>','grain_table_link_field_<?=$id?>',this); return false;" rel="<?=$val?>">x</a>
...
?><input type="text" name="<?=$name?>[]" value="<?=htmlspecialchars($val)?>" /><?
endforeach;
else:
