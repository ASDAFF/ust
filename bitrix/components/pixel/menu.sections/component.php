...
if ($this->StartResultCache(FALSE, $arParams["PARENT_ID"]))
{
    ...
    else
    {
        ...
        {
            $rsParentSection = CIBlockSection::GetByID($arParams["PARENT_ID"]);
            $arParentSection = $rsParentSection->GetNext();
            $arFilter = array('IBLOCK_ID' => $arParams["IBLOCK_ID"], '>LEFT_MARGIN' => $arParentSection['LEFT_MARGIN'], '<RIGHT_MARGIN' => $arParentSection['RIGHT_MARGIN'], '>DEPTH_LEVEL' => $arParentSection['DEPTH_LEVEL'], '<DEPTH_LEVEL' => $arParentSection['DEPTH_LEVEL'] + $arParams["DEPTH_LEVEL"] + 1, "GLOBAL_ACTIVE" => "Y", "IBLOCK_ACTIVE" => "Y",); // выберет потомков без учета активности
        }
        ...
        //  print "<pre fssgfsg style='display:none'>";   
