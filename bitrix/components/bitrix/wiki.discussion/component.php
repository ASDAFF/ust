				...
				if (intval($log_id) > 0)
				{
					...
					if ($bSocNetLogRecordExists) //if existing record- add only newly added comment
					{
						...
						if (
							...
							&& $arMessage["AUTHOR_ID"] != $log_user_id
						)
						{
							...
							if (!$arUnFollower)
							{
								...
								$arTmp = CSocNetLogTools::ProcessPath(array("ELEMENT_URL" => $url), $log_user_id);
								$serverName = $arTmp["SERVER_NAME"];
								$url = $arTmp["URLS"]["ELEMENT_URL"];

								$arMessageFields["NOTIFY_TAG"] = "WIKI|COMMENT|".$arFields["PARAM2"];
