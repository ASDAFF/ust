...
$arResult["AJAX_CALL"] = (
	array_key_exists("bxajaxid", $_REQUEST) 
	|| array_key_exists("logajax", $_REQUEST) 
);
			...
			foreach($arPresetFilterTmp["FILTER"] as $tmp_id_2 => $filterTmp)
			{
				...
				)
				{
					...
					elseif (!is_array($filterTmp))
						$arPresetFilterTmp["FILTER"][$tmp_id_2] = $user_id;
					elseif (is_array($filterTmp))
						foreach($filterTmp as $tmp_id_3 => $valueTmp)
							if ($valueTmp == "#CURRENT_USER_ID#")
								$arPresetFilterTmp["FILTER"][$tmp_id_2][$tmp_id_3] = $user_id;
				}
			}
		}
