...
else
{
	...
	ob_start();
	?><script>if(!!window.addForumImagesShow){window.addForumImagesShow('<?=$id?>');}else{BX.loadScript(['<?=CUtil::GetAdditionalFileURL('/bitrix/js/forum/popup_image.js')?>'], function(){addForumImagesShow('<?=$id?>');});}</script><?
	?><img src="<?=$arParams["URL"]?>" id="<?=$id?>" border="0" onload="if(!!window.onForumImageLoad){window.onForumImageLoad(this, '<?=$arParams["WIDTH"]?>', '<?=$arParams["HEIGHT"]?>', '<?=$arParams["FAMILY"]?>');}" /><?
	$arParams["RETURN_DATA"] = ob_get_clean();
}
