			...
			<div class="feed-post-cont-wrap<?=$aditStyles?>" id="blg-post-img-<?=$arResult["Post"]["ID"]?>">
				...
				?><div class="feed-post-informers" id="blg-post-inform-<?=$arResult["Post"]["ID"]?>"><?
					...
					if(!in_array($arParams["TYPE"], array("DRAFT", "MODERATION"))):
						...
						?><span class="feed-inform-comments"><a href="<?=$arResult["Post"]["urlToPost"]?>" id="blog-post-addc-link-<?=$arResult["Post"]["ID"]?>"<?=(!$bHasComments ? " style=\"display:none;\"" : "")?>><?=GetMessage("BLOG_COMMENTS")?></a><span id="blog-post-addc-add-<?=$arResult["Post"]["ID"]?>"<?=($bHasComments ? " style=\"display:none;\"" : "")?>><?=GetMessage("BLOG_COMMENTS_ADD")?></span></span><?
					endif;
