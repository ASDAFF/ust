...
(function() {
	...
	top.SBPImpPostCounter = function(node, postId, params) {
		...
		this.onPullEvent = BX.delegate(function(module_id, command, params){
			if (module_id == "socialnetwork" && command == 'read' && !!params && params["POST_ID"] == this.postId)
			{
				if (!!params["data"])
				{
		...
		BX.addCustomEvent("onPullEvent", this.onPullEvent);
	}
