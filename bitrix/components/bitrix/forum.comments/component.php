...
$arParams["URL_TEMPLATES_PROFILE_VIEW"] = (str_replace(array("#user_id#", "#author_id#", "#AUTHOR_ID#"), "#USER_ID#", $arParams["URL_TEMPLATES_PROFILE_VIEW"]));
/***************** STANDART ****************************************/
...
if ($arParams['PERMISSION'] !== null && !$GLOBALS["USER"]->IsAdmin())
{
			...
			if ($db_res)
			{
				...
				while ($res = $db_res->GetNext())
				{
					...
					if (!empty($res["AVATAR"])) {
						...
						if (!!$rAuthor && ($arAuthor = $rAuthor->Fetch()) && $arAuthor["PERSONAL_PHOTO"] > 0)
							$avatarId = $arAuthor["PERSONAL_PHOTO"];
					}
					if ($avatarId > 0) {
						$res["AVATAR"] = array(
							"ID" => $avatarId,
