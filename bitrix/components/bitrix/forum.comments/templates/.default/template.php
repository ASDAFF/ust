...
<?					if ($arResult["SHOW_PANEL"] == "Y") 
					{
						if ($arResult["PANELS"]["MODERATE"] == "Y") { ?>
							<span class="separator"></span>
							<a rel="nofollow" onclick="this.href+='&sessid='+BX.bitrix_sessid()" href="<?=$res["URL"]["MODERATE"]?>" class="comments-button-small"><?=GetMessage((($res["APPROVED"] == 'Y') ? "F_HIDE" : "F_SHOW"))?></a>
<?						}?>
<?						if ($arResult["PANELS"]["DELETE"] == "Y") { ?>
							<span class="separator"></span>
							<a rel="nofollow" onclick="this.href+='&sessid='+BX.bitrix_sessid()" href="<?=$res["URL"]["DELETE"]?>" class="comments-button-small"><?=GetMessage("F_DELETE")?></a>
<?						}
					} ?>
