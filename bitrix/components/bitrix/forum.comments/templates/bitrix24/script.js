;(function(window){
	...
	window.__fcOnUCFormAfterShow = function(obj, text, data) {
		var post_data = {MID : obj.id[1], ENTITY_XML_ID : obj.id[0], ENTITY_TYPE : obj.entitiesId[obj.id[0]][0], ENTITY_ID : obj.entitiesId[obj.id[0]][1]};
		...
		var mpFormObj = window['PlEditor' + obj.form.id];
		if (!!mpFormObj && !!data)
		{
			...
			if (!!data["FILES"])
			{
				...
				if (!!arRes && !!mpFormObj.FController)
				{
