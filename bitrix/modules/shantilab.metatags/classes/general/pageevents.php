...
class shanti_PageEvents
{
    ...
    function setTags()
    {
        ...
        if ($rule)
        {
            ...
            foreach ($rule["TAGS"] as $tag)
            {
                ...
                if (toUpper($priority) == "MODULE")
                {
                    ...
                    {
                        $APPLICATION->SetPageProperty($tag["CODE"], $tag["FINAL_VALUE"]);
                        $arInfo[] = array(
       ...
       // global $SHANTILAB_METATAGS_DATA;
        $shantiPageInfo["TAGS"]["SETTED"] = $settedTags;
        $_SESSION["SHANTILAB_METATAGS_DATA"] = $shantiPageInfo;
       // $SHANTILAB_METATAGS_DATA = $shantiPageInfo;

        /* echo "<!--";
          echo '<div style="display:none" dda>';
          print_r($_SESSION["SHANTILAB_METATAGS_DATA"]);
          echo '</div>';
          echo "-->"; */
    }
