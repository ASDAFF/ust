	...
	function OnPanelCreateButtons()
	{
		...
		if($APPLICATION->arPanelButtons["edit"]["ID"] == "edit" && $TAGS_RIGHT == "W"){
			...
			$pageItem = array(
				...
				"ICON" => "panel-key",
				"ACTION" => "(new BX.CAdminDialog({'content_url':'/bitrix/admin/".Shantilab\Metatags\Data::getPageName("edit_key")."?lang=ru&bxpublic=Y&&path=".urlencode($pageUrl)."&site=".SITE_ID."&back_url=".urlencode($APPLICATION->getCurPageParam())."','width':'700','height':'400'})).Show()",
				"SORT" => 1000,
			);
		...
		if($APPLICATION->arPanelButtons["edit_section"]["ID"] == "edit_section" && $TAGS_RIGHT == "W"){
			...
			$sectionItem = array(
				...
				"ICON" => "panel-key",
				"ACTION" => "(new BX.CAdminDialog({'content_url':'/bitrix/admin/".Shantilab\Metatags\Data::getPageName("edit_key")."?lang=ru&bxpublic=Y&path=".urlencode($APPLICATION->getCurDir())."&site=".SITE_ID."&back_url=".urlencode($APPLICATION->getCurPageParam())."','width':'700','height':'400'})).Show()",
				"SORT" => 1000,
			);
		...
		$arMenu[] = array(
			...
			"ACTION" => "(new BX.CDialog({'content_url':'/bitrix/admin/".Shantilab\Metatags\Data::getPageName("setted_keys")."?lang=ru&site=".SITE_ID."&path=".$APPLICATION->GetCurPage()."&back_url=".$APPLICATION->GetCurPageParam()."','width':'1000','height':'600'})).Show()",
			"DEFAULT" => false,
		);
		...
		$arMenu[] = array(
			...
			"ACTION" => "(new BX.CDialog({'content_url':'/bitrix/admin/".Shantilab\Metatags\Data::getPageName("setted_tags")."?lang=ru&site=".SITE_ID."&path=".$APPLICATION->GetCurPage()."&back_url=".$APPLICATION->GetCurPageParam()."','width':'1000','height':'600'})).Show()",
			"DEFAULT" => false,
		);
