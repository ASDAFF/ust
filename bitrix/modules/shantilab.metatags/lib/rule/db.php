...
class DB extends Base{
	...
	public static function getForPage(){
		...
		$cacheTime = \COption::GetOptionInt('shantilab.metatags','TIME_CACHE_SQL_REQUEST');
		if($obCache->InitCache($cacheTime, $cacheId, $cacheDir)){
			...
			if ($RulesTableName && $PagesTableName && $TemplatesTableName)
				$sql = "
					...
					LEFT JOIN $PagesTableName P_2 ON (P_2.RULE_ID = R.ID and P_2.SHOW_ON_PAGE='N' and ('".$sqlHelper->ForSQL($url)."' like concat(P_2.PAGE, '%') or $realFilePath like concat(P_2.PAGE, '%')))
